...
Push-Location $Root
try {
    foreach ($PowerShellScript in @("scripts\build_converter_core.ps1", "scripts\package_release.ps1", "scripts\test_all.ps1", "products\clipboard-helper\install.ps1")) {
        $tokens = $null
        $parseErrors = $null
    ...
    if (-not $SkipPython) {
        ...
        & $Python -m py_compile products/clipboard-helper/md2docx_clip.py products/skill-one/package_skill.py products/skill-one/skill-one/scripts/docx_brain.py scripts/check_credentials.py

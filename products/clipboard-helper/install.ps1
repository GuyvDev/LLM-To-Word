...
$CoreSource = @(
    (Join-Path $SourceDir "md2docx-core.exe"),
    (Join-Path $SourceDir "..\..\dist\windows\md2docx-core.exe")
) | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
$RequiredFiles = @("md2docx_clip.py", "requirements.txt")
if (-not $CoreSource) { throw "Missing md2docx-core.exe. Run scripts\build_converter_core.ps1 from the repository root or use a packaged clipboard-helper release." }
...
Copy-Item -LiteralPath $CoreSource -Destination (Join-Path $InstallDir "md2docx-core.exe") -Force
...
& $VenvPython -c "import win32clipboard, win32com.client; print('Native-core runtime check passed.')"
